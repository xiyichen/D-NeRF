...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    times_json = json.load(open(os.path.join(basedir, 'time.json'), 'r'))
    times = [times_json[fp.split('/')[-1][:-4]] for fp in imgfiles]
    times = np.array(times)
    
    print('Loaded image data', imgs.shape, poses[:,-1,0])
    return poses, bds, imgs, times
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    poses, bds, imgs, times = _load_data(basedir, factor=factor) # factor=8 downsamples original imgs by 8x
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    return images, poses, times, bds, render_poses, i_test
